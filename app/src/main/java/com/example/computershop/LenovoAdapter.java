...
public class LenovoAdapter extends RecyclerView.Adapter<LenovoAdapter.ViewHolder> {
    ...
    @NonNull
    @Override
    public LenovoAdapter.ViewHolder onCreateViewHolder(@NonNull ViewGroup parent, int viewType) {
        final View view = inflater.inflate(R.layout.cardviewcomp,parent,false);
        return new LenovoAdapter.ViewHolder(view);
    }

    @Override
    public void onBindViewHolder(@NonNull final LenovoAdapter.ViewHolder holder, int position) {
        final ComputerTypeData computerTypeData=DesktopRepo.getInstance().getLenovoData().get(position);
    ...
    public int getItemCount() {
        return DesktopRepo.getInstance().getLenovoData().size();
    }
