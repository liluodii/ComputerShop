...
public class SonyAdapter extends RecyclerView.Adapter<SonyAdapter.ViewHolder> {
    ...
    @NonNull
    @Override
    public SonyAdapter .ViewHolder onCreateViewHolder(@NonNull ViewGroup parent, int viewType) {
        final View view = inflater.inflate(R.layout.cardviewcomp,parent,false);
        return new SonyAdapter .ViewHolder(view);
    }

    @Override
    public void onBindViewHolder(@NonNull final SonyAdapter .ViewHolder holder, int position) {
        final ComputerTypeData computerTypeData=DesktopRepo.getInstance().getSonyData().get(position);
    ...
    public int getItemCount() {
        return DesktopRepo.getInstance().getSonyData().size();
    }
